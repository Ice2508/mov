...
// Инициализация selectedDate с сегодняшней датой
let selectedDate = new Date().toISOString().split('T')[0]; // 2025-05-21
...
export function renderWeek(containerId, offset = 0) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const dow = currentDate.getDay(); // 0 - воскресенье
  ...
  // Активный первый день (по умолчанию)
  const firstDay = container.querySelector('.calendar__day:not(.calendar__day--next)');
  if (firstDay) {
    firstDay.classList.add('calendar__day-active');
    selectedDate = firstDay.dataset.date; // установить начальную дату
  }
}

// Устанавливает обработчики кликов и сохраняет дату
export function calendarClick() {
  ...
  calendarDays.forEach(day => {
    day.addEventListener('click', () => {
      ...
      selectedDate = day.dataset.date; // сохраняем выбранную дату
      localStorage.setItem('date', JSON.stringify(selectedDate)); // Сохраняем в localStorage
      loadDataAndUse();
    });
  });
}

// Геттер: вернуть текущую выбранную дату
